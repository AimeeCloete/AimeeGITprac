...
<br> *Post-fire, plants will have lots of range in which to grow and thus will grow faster immediately after the fire. At a certain point density dependent processes will limit growth which in turn will limit NDVI.* 
...
<br> *The simple model has a higher alpha value, lower gamma value and higher lambda value than the full (more complex) model*
...
<br> *The full model is better because it has a lower residual sum of squares, therefore fits the data better than the simple model.*

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4:*

<br> *There are 2 degrees of freedom in this ANOVA, this is the difference between the number of paramters between the models. This is 2 because the full model has 2 more (additional) parameters which have been added that are not in the simple model. The parameters are "A" and "phi".*
...
<br> *There is a preferred model: The full model is preferred because the AIC value is lower than the AIC of the simple model with a difference more than 3 so the difference is significant at the 5% threshold.*
...
<br> *The AIC function is used to compare models fitted by maximum likelihood to the same data.  Therefore we compare the AIC values of the NLS models and AIC values of the MLE models because the AIC values are calculated differently so they are not of the same range and unable to be compared.*
